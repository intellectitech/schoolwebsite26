...
if ($slug) {
    ...
    if ($pdo) {
        try {
            $stmt = $pdo->prepare("SELECT * FROM news_posts WHERE slug = :slug AND is_published = 1 LIMIT 1");
...
if ($pdo) {
    try {
        $stmt = $pdo->query("SELECT * FROM news_posts WHERE is_published = 1 ORDER BY published_at DESC");
        $posts = $stmt->fetchAll();
