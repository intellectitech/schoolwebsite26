...
<head>
  ...
  <meta name="description" content="Latest news, events and announcements from Uganda Martyrs Primary School, Namugongo.">
...
<section class="page-hero">
  <div class="container reveal">
    ...
    <p class="lead">Term updates, pupil achievements, upcoming events and school news — everything that matters to our community, in one place.</p>
  </div>
    ...
    <div class="news-featured-card">
      <div class="news-feat-img" aria-hidden="true">
        <?php if ($featured['featured_image'] && file_exists(__DIR__ . '/' . $featured['featured_image'])): ?>
          <img src="<?= htmlspecialchars($featured['featured_image']) ?>" alt="<?= htmlspecialchars($featured['title']) ?>" style="width:100%;height:100%;position:absolute;inset:0;object-fit:cover">
        <?php else: ?>
        <svg viewBox="0 0 600 400" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice">
          ...
          <!-- hill layers -->
          <path d="M0,280 C120,230 280,220 400,260 C500,288 560,270 600,260 L600,400 L0,400 Z" fill="#2F6B4F" opacity="0.7"/>
          <path d="M0,320 C180,270 340,290 500,310 C560,318 590,315 600,312 L600,400 L0,400 Z" fill="#A6402E" opacity="0.75"/>
          <!-- sun -->
          <circle cx="300" cy="130" r="54" fill="#F2B705"/>
          <g stroke="#F2B705" stroke-width="4" stroke-linecap="round" opacity="0.5">
            <line x1="300" y1="56" x2="300" y2="40"/><line x1="300" y1="220" x2="300" y2="204"/>
            <line x1="218" y1="130" x2="202" y2="130"/><line x1="398" y1="130" x2="382" y2="130"/>
            <line x1="242" y1="72" x2="231" y2="61"/><line x1="369" y1="199" x2="358" y2="188"/>
            <line x1="358" y1="72" x2="369" y2="61"/><line x1="231" y1="199" x2="242" y2="188"/>
          </g>
      ...
      <div class="news-feat-body">
        <span class="news-tag news-tag-gold" style="background:<?= htmlspecialchars($featured['cat_color'] ?? '#F2B705') ?>"><?= htmlspecialchars($featured['cat_name'] ?? 'News') ?></span>
        ...
        <a href="article.php?slug=<?= urlencode($featured['slug']) ?>" class="btn btn-primary" style="align-self:flex-start">Read the full story →</a>
      </div>
    </div>
  </div>
</section>
    ...
    <?php if ($articles): ?>
          ...
          <div class="news-card">
            <div class="news-card-img">
              <?php if ($article['featured_image'] && file_exists(__DIR__ . '/' . $article['featured_image'])): ?>
                <img class="card-img" src="<?= htmlspecialchars($article['featured_image']) ?>" alt="<?= htmlspecialchars($article['title']) ?>">
              <?php else: ?>
      ...
      <?php if ($totalPages > 1): ?>
        <div class="news-pagination">
          <?php if ($page > 1): ?>
            <a class="pagination-btn" href="news.php?page=<?= $page - 1 ?><?= $catSlug ? '&category=' . urlencode($catSlug) : '' ?>" aria-label="Previous page">←</a>
          <?php endif; ?>
          <?php for ($i = 1; $i <= $totalPages; $i++): ?>
            <a class="pagination-btn <?= $i === $page ? 'active' : '' ?>"
               href="news.php?page=<?= $i ?><?= $catSlug ? '&category=' . urlencode($catSlug) : '' ?>"
               <?= $i === $page ? 'aria-current="page"' : '' ?>><?= $i ?></a>
          <?php endfor; ?>
          <?php if ($page < $totalPages): ?>
            <a class="pagination-btn" href="news.php?page=<?= $page + 1 ?><?= $catSlug ? '&category=' . urlencode($catSlug) : '' ?>" aria-label="Next page">→</a>
          <?php endif; ?>
        </div>
      <?php endif; ?>

    <?php else: ?>
      <p style="color:var(--ink-soft);padding:2rem 0">No news articles found<?= $catSlug ? ' in this category' : '' ?> yet.</p>
    <?php endif; ?>
...
<section class="newsletter-section" id="newsletter">
  ...
  <div class="container reveal">
    ...
    <p>We send a short newsletter at the end of each term — results, upcoming events, and a word from the head teacher. No spam, unsubscribe any time.</p>

    <?php if ($flash['success']): ?>
      <div class="alert alert-success" style="margin-top:1.2em;max-width:52ch"><?= htmlspecialchars($flash['success']) ?></div>
    <?php endif; ?>
    <?php if ($flash['errors']): ?>
      <div class="alert alert-error" style="margin-top:1.2em;max-width:52ch">
        <?php foreach ($flash['errors'] as $err): ?><p><?= htmlspecialchars($err) ?></p><?php endforeach; ?>
      </div>
    <?php endif; ?>
...
<section class="cta-band">
  ...
  <div class="container reveal">
    ...
    <p>Reading about us is a start — but nothing beats a morning in the compound, hearing the choir and meeting the teachers.</p>
