...
if ($pdo) {
    try {
        $stmt = $pdo->query("SELECT * FROM news_posts WHERE is_published = 1 ORDER BY published_at DESC LIMIT 3");
        $latestNews = $stmt->fetchAll();
    } catch (Exception $e) { $latestNews = []; }
}
...
if ($pdo) {
    try {
        $stmt = $pdo->query("SELECT * FROM gallery_images ORDER BY uploaded_at DESC LIMIT 6");
        $galleryPreview = $stmt->fetchAll();
    } catch (Exception $e) { $galleryPreview = []; }
}
...
if ($pdo) {
    try {
        ...
        $upcomingEventsPreview = $stmt->fetchAll();
    } catch (Exception $e) { $upcomingEventsPreview = []; }
}
...
<?php if (!empty($upcomingEventsPreview)): ?>
<section>
  <div class="container">
    <div class="section-header">
      <span class="eyebrow">Save the Date</span>
      <h2>Upcoming Events</h2>
      <p>Key dates on the school calendar parents shouldn't miss.</p>
    </div>
    <div class="grid grid-3">
      <?php foreach ($upcomingEventsPreview as $ev):
      ...
      <div class="card">
        <div class="card-body" style="display:flex; gap:16px; align-items:flex-start;">
          ...
          <div>
            ...
            <p style="margin-bottom:0; font-size:0.88rem;"> <?php echo htmlspecialchars($ev['location']); ?></p>
          </div>
        </div>
      </div>
      <?php endforeach; ?>
    ...
    <div class="text-center" style="margin-top:34px;">
      <a href="events.php" class="btn btn-blue">View All Events</a>
    </div>
  </div>
</section>
<?php endif; ?>
