...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_inquiry'])) {
    ...
    if (empty($errors)) {
        if ($pdo) {
            try {
                $stmt = $pdo->prepare("INSERT INTO admissions_inquiries
                    (parent_name, parent_email, parent_phone, child_name, desired_class, message)
                    VALUES (:parent_name, :parent_email, :parent_phone, :child_name, :desired_class, :message)");
                $stmt->execute([
            ...
            $errors[] = "Sorry, we can't reach our systems right now. Please try again shortly or call us directly.";
        }
    }
}
