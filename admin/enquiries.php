...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        tr.status-new { background: rgba(255,107,107,0.03); }
        ...
        .status-badge.declined { background: rgba(200,200,200,0.15); color: #999; border: 1px solid rgba(200,200,200,0.1); }
        ...
        .btn-status.new { background: rgba(255,107,107,0.15); color: #FF6B6B; border: 1px solid rgba(255,107,107,0.1); }
        .btn-status.contacted { background: rgba(0,123,255,0.15); color: #4d9fff; border: 1px solid rgba(0,123,255,0.1); }
        .btn-status.enrolled { background: rgba(0,200,83,0.15); color: #00C853; border: 1px solid rgba(0,200,83,0.1); }
        .btn-status.declined { background: rgba(200,200,200,0.15); color: #999; border: 1px solid rgba(200,200,200,0.1); }
        ...
        .stats-summary .stat { padding: 12px 24px; background: #fff; border-radius: 14px; border: 1px solid #e0e0e0; box-shadow: 0 5px 30px rgba(0,0,0,0.05); }
        .stats-summary .stat .num { font-size: 1.5rem; font-weight: 800; color: #0a0a0a; }
        .stats-summary .stat .label { font-size: 0.8rem; color: #999; }
...
<div class="admin-wrapper">
    ...
    <main class="admin-content">
        ...
        <div class="stats-summary">
            ...
            <div class="stat">
                <div class="num" style="color:#FF6B6B;"><?= $newCount ?></div>
                <div class="label">New (Needs Attention)</div>
            </div>
            <div class="stat">
                <div class="num" style="color:#00C853;"><?= $pdo->query("SELECT COUNT(*) FROM admissions_enquiries WHERE status = 'enrolled'")->fetchColumn() ?></div>
                        ...
                        <?php foreach ($enquiries as $enq): ?>
                            <tr class="status-<?= $enq['status'] ?>">
                                <td>
                                    ...
                                    <span style="font-size:0.8rem;color:#999;"><i class="fas fa-phone"></i> <?= clean($enq['parent_phone']) ?></span>
                                    <?php if ($enq['parent_email']): ?>
                                        <br><span style="font-size:0.8rem;color:#999;"><?= clean($enq['parent_email']) ?></span>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <?= clean($enq['student_name']) ?>
                                    <?php if ($enq['ple_aggregate']): ?>
                                        <br><span style="font-size:0.8rem;color:#999;">PLE: <?= $enq['ple_aggregate'] ?></span>
                                    <?php endif; ?>
                                </td>
                                <td><span style="font-weight:600;color:#00C853;"><?= clean($enq['entry_level']) ?></span></td>
                                <td>
                                    <span class="status-badge <?= $enq['status'] ?>">
                                        <?= ucfirst($enq['status']) ?>
                                    </span>
                                    <?php if ($enq['current_school']): ?>
                                        <br><span style="font-size:0.7rem;color:#999;"><?= clean($enq['current_school']) ?></span>
                                    <?php endif; ?>
                                </td>
                                ...
                                <td>
                                    ...
                                    <?php if ($enq['message']): ?>
                                        <div style="font-size:0.75rem;color:#999;margin-top:5px;max-width:200px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
                                            "<?= clean($enq['message']) ?>"
                                        </div>
                                    <?php endif; ?>
                                </td>
