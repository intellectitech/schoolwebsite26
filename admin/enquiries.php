  ...
  <div class="admin-shell">
    ...
    <main class="admin-main">
      <header class="admin-topbar">
        ...
        <p><?= count($enquiries) ?>
          enquir<?= count($enquiries) === 1 ? 'y' : 'ies' ?><?= $statusFilter ? ' — ' . htmlspecialchars(ucfirst($statusFilter)) : '' ?>
        </p>
      </header>
      ...
      <?php if ($viewing): ?>
        <section class="admin-panel admin-message-detail">
          ...
          <dl class="admin-detail-list">
            ...
            <dd><?= htmlspecialchars($viewing['parent_name']) ?></dd>
            <dt>Phone</dt>
            <dd><?= htmlspecialchars($viewing['parent_phone']) ?></dd>
            <?php if ($viewing['parent_email']): ?>
              <dt>Email</dt>
              <dd><a
                  href="mailto:<?= htmlspecialchars($viewing['parent_email']) ?>"><?= htmlspecialchars($viewing['parent_email']) ?></a>
              </dd><?php endif; ?>
            <?php if ($viewing['current_school']): ?>
              <dt>Current school</dt>
              <dd><?= htmlspecialchars($viewing['current_school']) ?></dd><?php endif; ?>
            <dt>Received</dt>
            <dd><?= date('l, d F Y', strtotime($viewing['created_at'])) ?></dd>
          </dl>
          <?php if ($viewing['message']): ?>
            <p class="admin-message-body"><?= nl2br(htmlspecialchars($viewing['message'])) ?></p><?php endif; ?>
          ...
          <form method="POST" action="enquiries.php" class="admin-status-form">
            ...
            <div class="field">
              ...
              <textarea id="admin_notes" name="admin_notes"
                rows="3"><?= htmlspecialchars($viewing['admin_notes']) ?></textarea>
            </div>
      ...
      <div class="admin-filter-tabs">
        ...
        <?php foreach ($validStatuses as $s): ?>
          <a href="enquiries.php?status=<?= $s ?>"
            class="cat-tab <?= $statusFilter === $s ? 'active' : '' ?>"><?= ucfirst($s) ?></a>
        <?php endforeach; ?>
      </div>
