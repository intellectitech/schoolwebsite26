...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    ...
    try {
        ...
        $logStmt = $pdo->prepare("
            ...
            VALUES (?, 'deleted_staff', 'staff', ?, 'Deleted staff member', ?)
        ");
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-toggle.active { background: rgba(76,175,80,0.15); color: #4CAF50; border: 1px solid rgba(76,175,80,0.1); }
        .btn-toggle.active:hover { background: rgba(76,175,80,0.25); }
        .btn-toggle.inactive { background: rgba(255,0,0,0.15); color: #ff6b6b; border: 1px solid rgba(255,0,0,0.1); }
        .btn-toggle.inactive:hover { background: rgba(255,0,0,0.25); }
        ...
        .status-badge.active { background: rgba(76,175,80,0.15); color: #4CAF50; border: 1px solid rgba(76,175,80,0.1); }
        .status-badge.inactive { background: rgba(255,0,0,0.15); color: #ff6b6b; border: 1px solid rgba(255,0,0,0.1); }

        .management-badge { display: inline-block; padding: 2px 10px; border-radius: 50px; font-size: 0.65rem; font-weight: 700; background: rgba(255,107,0,0.2); color: #FF6B00; border: 1px solid rgba(255,107,0,0.1); }
        ...
        <div class="table-container">
                            ...
                            <tr>
                                <td>
                                    <?php if ($member['is_management']): ?>
                                        <span class="management-badge"><i class="fas fa-star"></i> Leadership</span><br>
                                    <?php endif; ?>
                                ...
                                <td>
                                    <div class="actions">
                                        ...
                                        <a href="?toggle=<?= $member['id'] ?>" class="btn-toggle <?= $member['is_active'] ? 'active' : 'inactive' ?>" onclick="return confirm('Toggle staff status?')">
                                            <?= $member['is_active'] ? '<i class="fas fa-eye"></i> Hide' : '<i class="fas fa-eye-slash"></i> Show' ?>
                                        </a>
                                        <a href="?delete=<?= $member['id'] ?>" class="btn-delete" onclick="return confirm('Delete this staff member?')"><i class="fas fa-trash"></i> Delete</a>
                                    </div>
                                </td>
                            </tr>
                        <?php endforeach; ?>
            ...
            <?php else: ?>
                <div class="no-data">
                    ...
                    <p>No staff members yet. <a href="add-staff.php" style="color:#FF6B00;">Add your first staff member</a></p>
                </div>
            <?php endif; ?>
        </div>
    </main>
