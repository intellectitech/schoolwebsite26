...
$where  = '';
if ($status === 'published') $where = 'WHERE is_published = 1';
if ($status === 'draft')     $where = 'WHERE is_published = 0';
...
<div class="admin-shell">
  ...
  <main class="admin-main">
    ...
    <?php if ($flash['success']): ?><div class="alert alert-success"><?= htmlspecialchars($flash['success']) ?></div><?php endif; ?>
    <?php if ($flash['errors']): ?>
      <div class="alert alert-error"><ul><?php foreach ($flash['errors'] as $err): ?><li><?= htmlspecialchars($err) ?></li><?php endforeach; ?></ul></div>
    <?php endif; ?>
    ...
    <div class="news-cat-tabs" role="group" aria-label="Filter articles by status">
      ...
      <a href="news.php?status=published" class="cat-tab <?= $status === 'published' ? 'active' : '' ?>">Published (<?= $counts['published'] ?>)</a>
      <a href="news.php?status=draft" class="cat-tab <?= $status === 'draft' ? 'active' : '' ?>">Drafts (<?= $counts['draft'] ?>)</a>
    </div>

    <section class="admin-panel">
      <?php if ($articles): ?>
        <table class="admin-table admin-table-news">
          <thead><tr><th></th><th>Title</th><th>Category</th><th>Status</th><th>Published</th><th>Views</th><th></th></tr></thead>
          <tbody>
            <?php foreach ($articles as $a): ?>
              ...
              <tr>
                <td>
                  ...
                  <?php else: ?>
                    <span class="admin-thumb admin-thumb-color" style="background:<?= htmlspecialchars($a['cat_color'] ?? '#16233D') ?>"></span>
                  <?php endif; ?>
                </td>
                <td>
                  <?= htmlspecialchars($a['title']) ?>
                  <?php if ($a['is_featured']): ?><span class="admin-badge admin-badge-featured">★ Featured</span><?php endif; ?>
                </td>
                <td><span class="admin-badge" style="background:<?= htmlspecialchars($a['cat_color'] ?? '#999') ?>;color:#fff"><?= htmlspecialchars($a['cat_name'] ?? '—') ?></span></td>
                <td><span class="admin-badge admin-badge-<?= $a['is_published'] ? 'published' : 'draft' ?>"><?= $a['is_published'] ? 'Published' : 'Draft' ?></span></td>
                ...
                <td class="admin-table-actions">
                  ...
                  <form method="POST" action="news.php<?= $status !== 'all' ? '?status=' . urlencode($status) : '' ?>" onsubmit="return confirm('Delete this article permanently? This cannot be undone.');">
                    <?= csrfField() ?>
                    <input type="hidden" name="id" value="<?= (int) $a['id'] ?>">
        ...
        <?php if ($totalPages > 1): ?>
          <div class="news-pagination">
            <?php for ($i = 1; $i <= $totalPages; $i++): ?>
              <a class="pagination-btn <?= $i === $page ? 'active' : '' ?>" href="news.php?page=<?= $i ?><?= $status !== 'all' ? '&status=' . urlencode($status) : '' ?>"><?= $i ?></a>
            <?php endfor; ?>
          </div>
        <?php endif; ?>
      <?php else: ?>
    ...
    <section class="admin-panel admin-panel-narrow">
      ...
      <ul class="admin-category-list">
        <?php foreach ($categories as $cat): ?>
          <li><span class="admin-color-dot" style="background:<?= htmlspecialchars($cat['color']) ?>"></span><?= htmlspecialchars($cat['name']) ?></li>
        <?php endforeach; ?>
      </ul>
