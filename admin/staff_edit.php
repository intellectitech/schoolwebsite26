...
if (!empty($_GET['id']) && $pdo) {
    try {
        ...
        $row = $stmt->fetch();
        if ($row) { $member = $row; $editing = true; }
    } catch (Exception $e) {}
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors) && $pdo) {
        try {
            if ($existingId) {
                $stmt = $pdo->prepare("UPDATE staff SET full_name=:name, role_title=:role, bio=:bio, photo_path=:photo, display_order=:ord WHERE id=:id");
                $stmt->execute([':name' => $full_name, ':role' => $role_title, ':bio' => $bio, ':photo' => $photoPath, ':ord' => $display_order, ':id' => $existingId]);
                $_SESSION['flash'] = ['type' => 'success', 'message' => 'Staff member updated successfully.'];
            } else {
                $stmt = $pdo->prepare("INSERT INTO staff (full_name, role_title, bio, photo_path, display_order) VALUES (:name, :role, :bio, :photo, :ord)");
                $stmt->execute([':name' => $full_name, ':role' => $role_title, ':bio' => $bio, ':photo' => $photoPath, ':ord' => $display_order]);
                $_SESSION['flash'] = ['type' => 'success', 'message' => 'Staff member added successfully.'];
            }
            header('Location: staff.php');
            exit;
        } catch (Exception $e) {
            $errors[] = "Could not save the staff member. Please try again.";
        }
    } elseif (!$pdo) {
