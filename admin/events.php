...
if ($when === 'past') $where = 'WHERE event_date < CURDATE()';
if ($when === 'all')  $where = '';
...
<div class="admin-shell">
  ...
  <main class="admin-main">
    ...
    <div class="news-cat-tabs" role="group" aria-label="Filter events by date">
      <a href="events.php?when=upcoming" class="cat-tab <?= $when === 'upcoming' ? 'active' : '' ?>">Upcoming (<?= $counts['upcoming'] ?>)</a>
      <a href="events.php?when=past" class="cat-tab <?= $when === 'past' ? 'active' : '' ?>">Past (<?= $counts['past'] ?>)</a>
      <a href="events.php?when=all" class="cat-tab <?= $when === 'all' ? 'active' : '' ?>">All (<?= $counts['all'] ?>)</a>
    </div>

    <section class="admin-panel">
      <?php if ($events): ?>
        <table class="admin-table admin-table-news">
          <thead><tr><th></th><th>Event</th><th>Location</th><th>Date &amp; Time</th><th>Status</th><th></th></tr></thead>
          <tbody>
            <?php foreach ($events as $ev): ?>
              ...
              <tr>
                ...
                <td>
                  ...
                  <span style="color:var(--ink-soft);font-size:0.85em"><?= date('g:ia', strtotime($ev['start_time'])) ?> – <?= date('g:ia', strtotime($ev['end_time'])) ?></span>
                </td>
                <td><span class="admin-badge admin-badge-<?= $ev['is_published'] ? 'published' : 'draft' ?>"><?= $ev['is_published'] ? 'Published' : 'Draft' ?></span></td>
                <td class="admin-table-actions">
                  ...
                  <form method="POST" action="events.php<?= $when !== 'upcoming' ? '?when=' . urlencode($when) : '' ?>" onsubmit="return confirm('Delete this event permanently? This cannot be undone.');">
                    <?= csrfField() ?>
                    <input type="hidden" name="id" value="<?= (int) $ev['id'] ?>">
      ...
      <?php else: ?>
        <p class="admin-empty">No <?= $when === 'past' ? 'past' : ($when === 'all' ? '' : 'upcoming') ?> events yet. <a href="event-form.php">Add one →</a></p>
      <?php endif; ?>
    </section>
  </main>
</div>
