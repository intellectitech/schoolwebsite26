  ...
  <div class="admin-shell">
    ...
    <main class="admin-main">
      ...
      <section class="admin-panel">
        ...
        <form method="POST" action="news-form.php<?= $editId ? '?edit=' . $editId : '' ?>" enctype="multipart/form-data"
          class="admin-form">
          <?= csrfField() ?>

          <div class="field">
            <label for="title">Title</label>
            <input id="title" name="title" type="text" required maxlength="390"
              value="<?= htmlspecialchars($v('title')) ?>"
              placeholder="e.g. Primary Seven Pupils Visit the Martyrs Shrine">
          </div>

          <div class="field-row">
            <div class="field">
              <label for="slug">URL slug <span style="text-transform:none;letter-spacing:normal">(optional —
                  auto-created from the title if left blank)</span></label>
              <input id="slug" name="slug" type="text" value="<?= htmlspecialchars($v('slug')) ?>"
            ...
            <div class="field">
              ...
              <select id="category_id" name="category_id" required>
                ...
                <?php foreach ($categories as $cat): ?>
                  <option value="<?= $cat['id'] ?>" <?= (int) $v('category_id') === (int) $cat['id'] ? 'selected' : '' ?>>
          ...
          <div class="field">
            <label for="excerpt">Short summary <span style="text-transform:none;letter-spacing:normal">(optional — used
                on listing cards; auto-created from the story if left blank)</span></label>
            <textarea id="excerpt" name="excerpt" rows="2"
              maxlength="500"><?= htmlspecialchars($v('excerpt')) ?></textarea>
          </div>
          ...
          <div class="field">
            <label for="featured_image">Photo <span style="text-transform:none;letter-spacing:normal">(optional — JPG,
                PNG or WEBP, up to 3MB. If left blank, an illustrated placeholder is used instead.)</span></label>
          ...
          <div class="field-row">
            <div class="field">
              ...
              <select id="status" name="status">
                <option value="draft" <?= $v('status', 'draft') === 'draft' ? 'selected' : '' ?>>Draft (not visible on the
                  site)</option>
