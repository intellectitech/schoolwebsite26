...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($title === '')                                    $errors[] = 'Please give the event a title.';
    if (mb_strlen($title) > 290)                           $errors[] = 'That title is too long.';
    if ($description === '')                               $errors[] = 'Please add a short description.';
    ...
    if (!$errors && $endTime <= $startTime)                $errors[] = 'The end time must be after the start time.';
    ...
    if ($editId) {
        $pdo->prepare(
            ...
            $startDateTime, $endDateTime, $imagePath, $isPublished,
            $editId,
        ]);
...
$v = function ($key, $default = '') use ($f, $event) {
    ...
    if ($event) return $event[$key] ?? $default;
    return $default;
};
?>
<!DOCTYPE html>
...
<div class="admin-shell">
  ...
  <main class="admin-main">
    ...
    <section class="admin-panel">
      ...
      <form method="POST" action="event-form.php<?= $editId ? '?edit=' . $editId : '' ?>" enctype="multipart/form-data" class="admin-form">
        <?= csrfField() ?>

        <div class="field">
          <label for="title">Event title</label>
          <input id="title" name="title" type="text" required maxlength="290" value="<?= htmlspecialchars($v('title')) ?>" placeholder="e.g. Annual Sports Day">
        </div>

        <div class="field">
          <label for="description">Description</label>
          <textarea id="description" name="description" rows="4" required><?= htmlspecialchars($v('description')) ?></textarea>
        </div>

        <div class="field">
          <label for="location">Location</label>
          <input id="location" name="location" type="text" required maxlength="200" value="<?= htmlspecialchars($v('location')) ?>" placeholder="e.g. School Grounds">
        </div>
        ...
        <div class="field">
          <label for="featured_img">Photo <span style="text-transform:none;letter-spacing:normal">(optional — JPG, PNG or WEBP, up to 3MB)</span></label>
          ...
          <?php endif; ?>
          <input id="featured_img" name="featured_img" type="file" accept=".jpg,.jpeg,.png,.webp,image/jpeg,image/png,image/webp">
        </div>
