...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && isset($_POST['id'])) {
    ...
    // Aligns with your database ENUM schema ('enrolled' or 'declined')
    $newStatus = ($action === 'approve') ? 'enrolled' : 'declined';
...
// 3. Fetch applications along with their uploaded academic documents using a JOIN
try {
    $sql = "SELECT 
                e.id AS enquiry_id,
                e.parent_name,
...
<head>
    ...
    <title>Admissions Review Dashboard</title>
    <style>
        :root {
            --bg-color: #f8fafc;
            --text-dark: #0f172a;
        ...
        * {
        ...
        .status-enrolled { background-color: #d1fae5; color: #065f46; }
...
<div class="dashboard-wrapper">
    <div class="dashboard-header">
        ...
        <div class="nav-shortcuts">
            <a href="approved.php" class="btn-nav">Approved List</a>
            <a href="declined.php" class="btn-nav">Declined List</a>
            <span class="counter-badge"><?= count($applications) ?> Total</span>
        </div>
    </div>
    ...
    <?php if (empty($applications)): ?>
        <div class="empty-state">
            ...
            <p>New application inquiries will populate here once parents apply.</p>
        </div>
    <?php else: ?>
            ...
            <?php foreach ($applications as $app): ?>
                <div class="app-card" id="app-card-<?= $app['enquiry_id'] ?>">
                    ...
                    <div class="app-card-header">
                        ...
                        <span id="status-tag-<?= $app['enquiry_id'] ?>" class="status-badge status-<?= $app['status'] ?: 'new' ?>">
                            <?= htmlspecialchars($app['status'] ?: 'new') ?>
                        </span>
                    </div>
                    ...
                    <div class="app-card-footer">
                        ...
                        <button class="btn-action btn-decline" onclick="updateStatus(<?= $app['enquiry_id'] ?>, 'decline')">
...
function updateStatus(enquiryId, action) {
    ...
    // Dynamically tracks back to whichever name this current file uses to execute code securely
    ...
    .then(data => {
        if (data.success) {
            const statusTag = document.getElementById(`status-tag-${enquiryId}`);
            statusTag.className = 'status-badge';
            
            if (data.newStatus === 'enrolled') {
                statusTag.classList.add('status-enrolled');
                statusTag.innerText = 'enrolled';
            } else if (data.newStatus === 'declined') {
                statusTag.classList.add('status-declined');
                statusTag.innerText = 'declined';
            }
        } else {
    ...
    .catch(error => {
        ...
        alert('An error occurred during process.');
    });
}
</script>
